...
const HelmRealmPage: React.FC = () => {
  const [selectedChallenge, setSelectedChallenge] = useState<string | null>(null);
  const { completedChallenges } = useContext(GameContext);
  
  const currentChallenges = helmChallenges.map(challenge => ({
    ...challenge,
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>⎈ Helm & Kubernetes 101</h1>
        <p style={styles.description}>
          Master the basics of Helm - the package manager for Kubernetes!
        </p>
      </div>
...
const styles = {
  ...
  progressFill: {
    ...
    background: 'linear-gradient(90deg, #0f1689 0%, #1e3a8a 100%)',
  },
  challengeList: {
    display: 'flex',
