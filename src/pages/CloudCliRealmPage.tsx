...
const CloudCliRealmPage: React.FC = () => {
  const [selectedChallenge, setSelectedChallenge] = useState<string | null>(null);
  const { completedChallenges } = useContext(GameContext);
  
  const currentChallenges = cloudCliChallenges.map(challenge => ({
    ...challenge,
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>☁️ Cloud CLI Mastery</h1>
        <p style={styles.description}>
          Master AWS and Google Cloud command-line tools - essential skills for modern cloud development!
        </p>
      </div>
...
export default CloudCliRealmPage;